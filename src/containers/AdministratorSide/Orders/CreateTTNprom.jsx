...
import { editOrderProm } from '../../../actions/ordersAction';
...
class CreateTTNprom extends Component {
  state = {
      ...
      methodProperties: {
        ...
        ServiceType:
          this.props.order.delivery.deliveryMethodId === 1
            ...
            : null,
        SeatsAmount: '1',
        Description: this.props.order.items.reduce(
  ...
  createTTNprom = () => {
    ...
    createNovaPoshtaTTN(this.state.ttnRequestObject)
      ...
      .then(
        (data) =>
          data &&
          editOrderProm (id, { ttn: data.IntDocNumber, status: 2 }).then((res) =>
    ...
    return (
      <>
        ...
        <Modal
          ...
          footer={[
            <Button form='createTTNprom' onClick={createTTNprom}>
              Создать ТТН
            </Button>,
          ]}
          okButtonProps={{ disabled: true }}
          width={750}>
          <form id='createTTNprom' onSubmit={createTTNprom}>
            <Tabs
              defaultActiveKey='3'
...
export default connect(mapStateToProps)(CreateTTNprom);
