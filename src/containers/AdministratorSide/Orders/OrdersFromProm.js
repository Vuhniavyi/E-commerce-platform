...
import { getOrdersProm, passToContractor } from '../../../actions/ordersAction';
import { statusListProm } from './statusListProm';
import CreateTTNprom from './CreateTTNprom';
...
const columns = [
  ...
  {
    title: 'Статус заказа',
    dataIndex: 'statusProm',
    key: 'status',
    render: (status, order) => {
      let selectedStatusProm = statusListProm.find(item => item.id === order.statusProm);
      let dates = [];
      let newHistory = [];
      ...
      return (
        <span className={styles.orderStatusInTable}>
          <span style={{ color: colors[status], fontWeight: 'bold' }}>{selectedStatusProm && selectedStatusProm.title}</span>
          <Popover content={(
            <div>
              <Timeline>
                {newHistory.map(item => {
                  const getStatus = (id) => {
                    let status = statusListProm.find(status => status.id === id);
                    return status.title;
                  };
...
class OrdersFromProm extends Component {
  ...
  getOrders1 = async ({ id, min_date, max_date, status, user_fio, user_phone }) => {
    ...
    const res1 = await getOrdersProm(url1);
  ...
  getOrders2 = async ({ id, min_date, max_date, status, user_fio, user_phone }) => {
    ...
    const res2 = await getOrdersProm(url2);
  ...
  getOrders3 = async ({ id, min_date, max_date, status, user_fio, user_phone }) => {
    ...
    const res3 = await getOrdersProm(url3);
...
export default connect(mapStateToProps, mapDispatchToProps)(OrdersFromProm);
